...
def encryption(m):
    ...
    '''
    return [[chr(col + ord('A')) for col in row] for row in m]
...
def decryption(m):
    ...
    '''
    return [[chr(col%26 + ord('A')) for col in row] for row in m]
...
def hill_cipher(text, key):
    ...
    inverse_factor = modinv(get_matrix_determinant(key_matrix) % 26, 26)
